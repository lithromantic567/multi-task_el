...
def guess_room_train(train_dataset,eval_dataset):
    ...
    for i in range(150):
        ...
        for data,data_d,label in train_dataloader: 
            ...
            type_idxes, token_probs,type_probs , sent = task(cur_obs_info,cur_obs_d_info)
        ...
        with open("mt1/train_acc.txt",'a') as fp:
            fp.write(str(acc_train)+'\n')
        with open("mt1/train_reward.txt",'a') as fp:
            fp.write(str(total_reward)+'\n')
        with open("mt1/train_loss_A.txt",'a') as fp:
            fp.write(str(total_loss_A)+'\n')  
        with open("mt1/train_loss_B.txt",'a') as fp:
        ...
        if acc_eval>best_eval:
            best_eval=acc_eval
            torch.save(task.state_dict(), "mt1/model.pth")
...
def guess_room_evaluate(eval_dataset,model):
    ...
    with torch.no_grad():
        ...
        for data,data_d,label in eval_dataloader:
            ...
            #ball=ball.to(torch.float32)
            #box=box.to(torch.float32)
            type_idxes, token_probs,type_probs , sent = model(cur_obs_info,cur_obs_d_info,  choose_method="greedy")
            for i in range(5,10):
                print(sent[i])
                print(type_idxes[i])
                print(label[i])
        ...
        with open("mt1/eval_acc.txt",'a') as f:
...
def task_test(test_dataset):
    ...
    model.load_state_dict(torch.load("mt1/model.pth"))
    model.eval()
        ...
        for message in s:
            ...
            for i in range(len(d)):
                if d[i]==message:
                    label1[tgt[i]]+=1
                    label2[pred[i]]+=1
            with open("mt1/pattern.txt",'a') as f:
