...
from MulTaskAgent import MulTaskModel
import torch
import pickle
import numpy as np
...
def guess_room_train(train_dataset1,eval_dataset1,train_dataset2,eval_dataset2):
    clear_file("Mul/Task1")
    clear_file("Mul/Task2")
    ...
    for i in range(100):
        ...
        process_result(i,tgt1,pred_type1,total_reward1,total_loss_A1,total_loss_B1,"Mul/Task1")
        process_result(i,tgt2,pred_type2,total_reward2,total_loss_A2,total_loss_B2,"Mul/Task2")
        
        task.eval()
        task.agentA.eval()
        task.agentB.eval()
        acc_eval1=guess_room_evaluate(eval_dataset1, task,guess_attribute="type",save_path="Mul/Task1")
        acc_eval2=guess_room_evaluate(eval_dataset2, task,guess_attribute="color",save_path="Mul/Task2")
        if acc_eval1>best_eval1:
            best_eval1=acc_eval1
            torch.save(task.state_dict(), "Mul/Task1/model.pth")
        if acc_eval2>best_eval2:
            best_eval2=acc_eval2
            torch.save(task.state_dict(), "Mul/Task2/model.pth")
...
if __name__ == "__main__":
    ...
    task_test(test_dataset1,"Mul/Task1","type")
    task_test(test_dataset2,"Mul/Task2","color")
